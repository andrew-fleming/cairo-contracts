...
fn test_constructor() {
    ...
    let public_key = Account::AccountABI::get_public_key(@state);
    assert_eq!(public_key, PUBKEY);

    let supports_isrc5 = Account::AccountABI::supports_interface(@state, ISRC5_ID);
    assert!(supports_isrc5);

    let supports_isrc6 = Account::AccountABI::supports_interface(@state, ISRC6_ID);
