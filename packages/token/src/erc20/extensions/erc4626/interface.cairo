...
pub trait IERC4626<TState> {
    ...
    /// MUST NOT panic.
    ///
    /// Note that this calculation MAY NOT reflect the “per-user” price-per-share, and instead should reflect
    ...
    /// MUST NOT panic.
    ///
    /// Note that any unfavorable discrepancy between `convert_to_shares` and `preview_deposit` SHOULD be considered
    ...
    ///
    /// Note that any unfavorable discrepancy between `convert_to_assets` and `preview_mint` SHOULD be considered
    ...
    ///
    /// MUST emit the `Deposit` event.
    /// MAY support an additional flow in which the underlying tokens are owned by the Vault contract before the mint execution,
    ...
    /// in the same transaction i.e. withdraw should return the same or fewer shares as preview_withdraw if called
    /// in the same transaction.
    /// MUST NOT account for withdrawal limits like those returned from max_withdraw and should always act as though
    /// the withdrawal would be accepted, regardless if the user has enough shares, etc.
    /// MUST be inclusive of withdrawal fees. Integrators should be aware of the existence of withdrawal fees.
    /// MUST not panic.
    ///
    /// Note that any unfavorable discrepancy between `convert_to_shares` and `preview_withdraw` SHOULD be considered
    /// slippage in share price or some other type of condition, meaning the depositor will lose assets by depositing.
    ...
    /// in the same transaction i.e. redeem should return the same or more assets as preview_redeem if called in the
    /// same transaction.
    /// MUST NOT account for redemption limits like those returned from max_redeem and should always act as though
    /// the redemption would be accepted, regardless if the user has enough shares, etc.
    /// MUST be inclusive of withdrawal fees. Integrators should be aware of the existence of withdrawal fees.
    /// MUST NOT panic.
    ///
    /// Note any unfavorable discrepancy between `convert_to_assets` and `preview_redeem` SHOULD be considered slippage
